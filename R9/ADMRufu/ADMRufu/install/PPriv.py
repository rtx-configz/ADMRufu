...
parser.add_argument("-p", "--port", help="Port number to use as proxy")
parser.add_argument("-t", "--texto", help="Custom message text")
...
if args.port:
    ...
    print(" You must enter the port that will be used as proxy...")
    sys.exit()
...
def test(HandlerClass=SimpleHTTPProxyHandler, ServerClass=ThreadingHTTPServer, protocol="HTTP/1.1"):
    ...
    print("Server: " + str(sa[0]) + " Port " + str(sa[1]))
    httpd.serve_forever()
