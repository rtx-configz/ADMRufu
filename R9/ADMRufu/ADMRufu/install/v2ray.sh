...
ins_v2r(){
    title "YOU ARE ABOUT TO INSTALL V2RAY!"
    print_center -ama "Installation may have some failures!\nPlease carefully observe the installation log.\nIt may contain information about some errors!\nand they should be corrected manually before\ncontinuing to use the script."
    enter
...
v2ray_tls(){
    ...
    if [[ ! "$(echo "$db"|grep '.crt')" = "" ]]; then
        ...
        title "SSL CERTIFICATE FOUND"
        echo -e "$(msg -azu "DOMAIN:") $(msg -ama "$DOMI")"
        echo -e "$(msg -azu "CERT:") $(msg -ama "$cert")"
        echo -e "$(msg -azu "KEY:")  $(msg -ama "$key")"
        msg -bar
        msg -ne " Continue using this certificate [Y/N]: " && read opcion_tls

        if [[ $opcion_tls = @(S|s|Y|y) ]]; then
...
removeV2Ray(){
    ...
    clear
    msg -bar
    print_center -verd "V2RAY REMOVED!"
    enter
    return 1
}

 v2ray_stream(){
    title "V2RAY PROTOCOLS"
    echo -e "\033[1;37m"
    v2ray stream
    msg -bar
    read foo
 ...
 port(){
    ...
    title "CONFIGURE V2RAY PORT"
    print_center -azu "current port: $(msg -ama "$port")"
    back
    in_opcion "New port"
    opcion=$(echo "$opcion" | tr -d '[[:space:]]')
    tput cuu1 && tput dl1
    if [[ -z "$opcion" ]]; then
        msg -ne " you must enter an option"
        sleep 2
        return
    elif [[ ! $opcion =~ $numero ]]; then
        msg -ne " you must enter only numbers"
        sleep 2
        return
    elif [[ "$opcion" = "0" ]]; then
        return
 ...
 alterid(){
    ...
    title "CONFIGURE V2RAY alterId"
    print_center -azu "current alterid: $(msg -ama "$aid")"
    back
    in_opcion "New alterid"
    opcion=$(echo "$opcion" | tr -d '[[:space:]]')
    tput cuu1 && tput dl1
    if [[ -z "$opcion" ]]; then
        msg -ne " you must enter an option"
 ...
 n_v2ray(){
    title "NATIVE V2RAY CONFIGURATION"
    echo -ne "\033[1;37m"
    v2ray
 }
 ...
 address(){
    ...
    title "CONFIGURE V2RAY address"
    print_center -azu "current: $(msg -ama "$add")"
    back
    in_opcion "New address"
    opcion=$(echo "$opcion" | tr -d '[[:space:]]')
    tput cuu1 && tput dl1
    if [[ -z "$opcion" ]]; then
        msg -ne " you must enter an option"
 ...
 host(){
    host=$(jq -r '.inbounds[].streamSettings.wsSettings.headers.Host' $config) && [[ $host = null ]] && host='no host'
    title "CONFIGURE V2RAY host"
    print_center -azu "Current: $(msg -ama "$host")"
    back
    in_opcion "New host"
    opcion=$(echo "$opcion" | tr -d '[[:space:]]')
    tput cuu1 && tput dl1
 ...
 path(){
    ...
    title "CONFIGURE V2RAY path"
    print_center -azu "Current: $(msg -ama "$path")"
    back
    in_opcion "New path"
    opcion=$(echo "$opcion" | tr -d '[[:space:]]')
    tput cuu1 && tput dl1
 ...
 reset(){
    title "RESTORING V2RAY SETTINGS"
    ...
    rm $config
    jq '.inbounds[].streamSettings += {"network":"ws","wsSettings":{"path": "/ADMRufu/","headers": {"Host": "example.com"}}}' < $temp > $config
    chmod 777 $config
    rm $temp
    sleep 2
    if [[ ! -z "$user" ]]; then
...
do
    ...
    msg -bar
    msg -ama "INSTALLATION"
    msg -bar3
    menu_func "$(msg -verd "INSTALL/RE-INSTALL V2RAY")" \
    "$(msg -verm2 "UNINSTALL V2RAY")\n$(msg -bar3)\n$(msg -ama "CONFIGURATION")\n$(msg -bar3)" \
    "SSL/TLS Certificate $(msg -ama "(native menu)")" \
    "Protocols $(msg -ama "(native menu)")" \
    "Port" \
    "alterId" \
    "V2ray Config $(msg -ama "(native menu)")\n$(msg -bar3)\n$(msg -ama "CLIENTS")\n$(msg -bar3)" \
    "Address" \
    "Host" \
    "Path\n$(msg -bar3)\n$(msg -ama "EXTRAS")\n$(msg -bar3)" \
    "Restore settings"
