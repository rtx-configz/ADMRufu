# ...existing code...
#!/bin/bash
# Connection limiter / monitor (English)
...
droppids(){
  ...
  for pid in $pids; do
    ...
    if [ $pidend ];then
       ...
       time_login=`echo $login |awk -F" " '{print $2"-"$1,$3}'`
       while [ ${#time_login} -lt 13 ]; do
    ...
    for i in `echo "$user_type"`; do
        ...
        if [[ $(date '+%s') -gt $(date '+%s' -d "$userDate") ]]; then
          if [[ $(passwd --status $user|cut -d ' ' -f2) = "P" ]]; then
            usermod -L $user
            echo "$user $(printf '%(%H:%M:%S)T') expired" >> /etc/ADMRufu/user/limit.log
          fi    
        fi
      done
}
