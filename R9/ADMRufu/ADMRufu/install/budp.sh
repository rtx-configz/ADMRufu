...
WantedBy=multi-user.target" > /etc/systemd/system/badvpn.service
    ...
    sleep 2
    [[ -z $(lsof -V -i tcp -P -n | grep -v "ESTABLISHED" |grep -v "COMMAND"|grep "badvpn-ud"|awk '{print $1}') ]] && print_center -verm2 "FAILED TO START" || print_center -verd "BADVPN STARTED" 
    sleep 1
else
    print_center -ama "STOPPING BADVPN"
    msg -bar
    systemctl stop badvpn &>/dev/null
    systemctl disable badvpn &>/dev/null
    rm /etc/systemd/system/badvpn.service
    sleep 2
    [[ -z $(lsof -V -i tcp -P -n | grep -v "ESTABLISHED" |grep -v "COMMAND"|grep "badvpn-ud"|awk '{print $1}') ]] && print_center -ama "BADVPN STOPPED" || print_center -verm2 "FAILED TO STOP"
    sleep 1  
fi
unset st_badvpn
enter
...
install(){
    title "INSTALLING BADVPN"
    if [[ ! -e /usr/bin/badvpn-udpgw ]]; then
        echo -ne "$(msg -azu " INSTALLING DEPENDENCIES...") "
        ...
        cd ${ADM_src}
        echo -ne "$(msg -azu " DOWNLOADING BADVPN......") "
        ...
        echo -ne "$(msg -azu " COMPILING BADVPN.......") "
